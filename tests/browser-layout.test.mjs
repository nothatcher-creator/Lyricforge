...
let server,chrome,port,debugPort,profile,ws,seq=0,pending=new Map();
...
before(async()=>{
  ...
  await waitHttp(`http://127.0.0.1:${debugPort}/json/version`);await connectPage();await cdp('Page.enable');await cdp('Runtime.enable');
});

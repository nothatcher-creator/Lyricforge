...
async function waitChromeDebug(url,tries=80){for(let i=0;i<tries;i++){try{const r=await fetch(url);if(r.ok)return r;}catch{}if(chromeSpawnError)throw new Error(`Chrome spawn failed: ${chromeSpawnError.message}`);if(chrome?.exitCode!==null)throw new Error(`Chrome exited before DevTools became ready (code ${chrome.exitCode}). stderr:\n${chromeStderr||'(empty)'}`);await sleep(100);}throw new Error(`Timed out: ${url}. Chrome exitCode=${chrome?.exitCode??'running'}. stderr:\n${chromeStderr||'(empty)'}`);}

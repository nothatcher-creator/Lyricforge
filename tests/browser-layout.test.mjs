...
test('portrait media import yields back to preview and play starts imported audio',async t=>{
  ...
  const hit=await evalJs(`(()=>{const e=document.querySelector('.preview-play'),r=e.getBoundingClientRect(),h=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);window.__lastAction=null;document.addEventListener('click',ev=>{window.__lastAction=ev.target.closest('[data-action]')?.dataset.action||ev.target.className||ev.target.tagName},{once:true,capture:true});return {wanted:{x:r.left+r.width/2,y:r.top+r.height/2},tag:h?.tagName,cls:h?.className,action:h?.closest('[data-action]')?.dataset.action||null};})()`);
  assert.equal(hit.action,'play',`Play coordinates are intercepted by ${JSON.stringify(hit)}`);
  await clickSelector('.preview-play');for(let i=0;i<20&&!await evalJs(`window.__LYRICFORGE__.audio.playing`);i++)await sleep(50);
  const playing=await evalJs(`({playing:window.__LYRICFORGE__.audio.playing,time:window.__LYRICFORGE__.audio.currentTimeUs(),ctx:window.__LYRICFORGE__.audio.ctx?.state,lastAction:window.__lastAction,button:document.querySelector('.preview-play')?.textContent})`);
  assert.equal(playing.lastAction,'play',`Trusted tap did not reach play handler: ${JSON.stringify(playing)}`);assert.equal(playing.playing,true,`Visible preview play button must start imported audio: ${JSON.stringify(playing)}`);assert.ok(playing.time>0,'Playback time must advance');
});
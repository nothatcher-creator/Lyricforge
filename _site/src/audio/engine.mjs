...
export function clipFadeGain(clip,timeUs){const local=Math.max(0,Number(timeUs)-(Number(clip?.startUs)||0)),duration=Math.max(1,(Number(clip?.endUs)||0)-(Number(clip?.startUs)||0)),fadeIn=Math.max(0,Math.min(duration,Number(clip?.fadeInUs)||0)),fadeOut=Math.max(0,Math.min(duration,Number(clip?.fadeOutUs)||0));let gain=1;if(fadeIn>0)gain=Math.min(gain,local/fadeIn);if(fadeOut>0)gain=Math.min(gain,(duration-local)/fadeOut);return Math.max(0,Math.min(1,gain));}
